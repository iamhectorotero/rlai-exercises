...
if __name__ == "__main__":
    ...
    for parameter_setting_i, parameter_setting in tqdm(enumerate(parameter_settings), total=len(parameter_settings)):
        for run_i in tqdm(range(N_RUNS)):
            ...
            sample_average_estimator = SampleAverageEstimator(action_value_initial_estimates=np.full(K, fill_value=0.0), epsilon=parameter_setting)
            weighted_estimator = WeightedEstimator(action_value_initial_estimates=np.full(K, fill_value=0.0), epsilon=0.1, alpha=parameter_setting)
            ucb = UCBEstimator(action_value_initial_estimates=np.full(K, fill_value=0.0), epsilon=0.1, alpha=0.1, c=parameter_setting)
            gradient_bandit = GradientBandit(action_value_initial_estimates=np.full(K, fill_value=0.0), alpha=parameter_setting)
